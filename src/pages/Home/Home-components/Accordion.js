...
export const Accordion = ({ faq }) => { //e prop obj lo each individual "accordian id" capture avvi each accordian state ni utilize cheskontadhi

  //destructing properties 
  const { question, answer } = faq;

  //creating state for each individual accordian
  const [show, setShow] = useState(false); //initialState is "false"
  ...
  return (
    <div>
      ...
      {show && ( //show is true then show this <div id="accordion-flush-body-1"></div> answer. otherwise not show
