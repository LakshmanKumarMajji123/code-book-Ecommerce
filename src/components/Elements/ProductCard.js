...
export const ProductCard = ({ product }) => { //access my prop obj "product"

  //deconstrcting the details
  const { id, name, overview, price, image_local, poster, rating, best_seller } = product;
 ...
 * <Link to={`products/${id}`} className="relative" >
        ...
        {best_seller && <span className="absolute top-4 left-2 px-2 bg-orange-500 bg-opacity-90 text-white rounded">Best Seller</span>}
        best_seller is "true" then show <span className="absolute top-4 left-2 px-2 bg-orange-500 bg-opacity-90 text-white rounded">Best Seller</span>
 
     we write (best_seller & <span></span>) instead of terinary operator (best_seller ? : )
*/

/**
 * ratings Task
 * ------
 * 1. create a component and write logic in it..
 * 2. That component returns some type of JSX for this "rating"
 */