...
export const DropdownLoggedIn = ({ setDropdown }) => {
    ...
    //when user click 'logout' button in dropdown of header then remove "token, id" in 'sessionstorage'
    function handleLogout() {
        sessionStorage.removeItem('token');
        sessionStorage.removeItem('userId');
        setDropdown(false);
        //after remove 'token, id' navigate to 'home' page
        navigate("/");
    }
    ...
    return (
        <div id="dropdownAvatar" className="select-none	absolute top-10 right-0 z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
            <div className="py-3 px-4 text-sm text-gray-900 dark:text-white">
                <div className="font-medium truncate">name@example.com</div>
            </div>
